...
public class Repository : IRepository
{
    ...
    public async Task<HttpResponseWrapper<TActionResponse>> PostAsync<T, TActionResponse>(string url, T model)
    {
        ...
        if (responseHttp.IsSuccessStatusCode)
        {
            var response = await UnserializeAnswerAsync<TActionResponse>(responseHttp, _jsonDefaultOptions);
    ...
    private async Task<T> UnserializeAnswerAsync<T>(HttpResponseMessage httpResponse, JsonSerializerOptions jsonSerializerOptions)
    {
